...
module Orangetheses
  describe Fetcher do
    ...
    describe '#_flatten_json' do
      subject {described_class.new.send(:flatten_json, rest_response).first}
      ...
      let(:rest_response) do
        [{
          ...
          "expand"=>["parentCollection", "parentCollectionList", "parentCommunityList", "bitstreams", "all"],
          ...
          "metadata"=>[
            ...
            {"key"=>"dc.identifier.uri", "value"=>"http://arks.princeton.edu/ark:/88435/#{id}", "language"=>nil},
      ...
      it "each key value of the metadata hash is a key in the record hash" do
        rest_response.first['metadata'].each do |m|
          expect(subject.has_key?(m['key'])).to be true
        end
      end
